...
public class Rule4 extends ExplorationRule {

    public Rule4(Cell[][] boardToExplore, int x, int y, int numberOfInitialGrains) {
    ...
    public boolean isSatisfied() {
        ...
        int probabilityX = AppConfiguration.getInstance().getProbabilityOfChange();
        int n = random.nextInt(100);

        List<Integer> idsOfMostFrequentGrains = new ArrayList<>();
        int currentMaxOfNeighboursType = 0;

        for (int i = 0; i < neighboursCounter.length; i++) {
            if (neighboursCounter[i] > currentMaxOfNeighboursType) {
                idsOfMostFrequentGrains.clear();
                currentMaxOfNeighboursType = neighboursCounter[i];
                idsOfMostFrequentGrains.add(i);
            } else if (neighboursCounter[i] == currentMaxOfNeighboursType && neighboursCounter[i] > 0) {
                idsOfMostFrequentGrains.add(i);
            }
        }
...
/*        boolean isNonZero = false;
        ...
        if (idsOfMostFrequentGrains.size() > 0 && n <= probabilityX) {
            //choose random Id depending on neighbours
            nextIdOfCurrentCell = idsOfMostFrequentGrains.get(random.nextInt(idsOfMostFrequentGrains.size()));

         /*   while (true) {
                int randomIndexToReturn = random.nextInt(neighboursCounter.length);

                if (neighboursCounter[randomIndexToReturn] > 0) {
                    nextIdOfCurrentCell = randomIndexToReturn;
                    break;
                }
            }*/
